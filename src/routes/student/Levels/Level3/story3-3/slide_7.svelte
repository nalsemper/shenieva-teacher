<script>
    ...
    image: "/converted/assets/LEVEL_3/STORY_3/pic6.webp"
    };
    ...
    $: audioSrc = (() => {
        ...
        let filename = sp === 'normal' ? 'LL 6.mp3' : 'LL6.mp3';
...
<div class="slide-container" bind:this={containerEl}>
    ...
    <div class="story-text" transition:fade>{$language === 'english' ? story.text.english : story.text.cebuano}</div>

    <audio bind:this={audioEl} src={audioSrc} on:play={() => { isPlaying = true; }} on:pause={() => { isPlaying = false; }} on:ended={exitStoryMode}></audio>
</div>

<style>
    .slide-container { width:100%; height:100%; display:flex; flex-direction:column; justify-content:space-between; align-items:center; gap:1rem; padding:1rem; box-sizing:border-box; animation: fadeIn 0.5s ease-in-out; position: relative; }
