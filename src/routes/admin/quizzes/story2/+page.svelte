<script lang="ts">
  ...
  async function fetchQuizzes() {
    try {
      const response = await fetch('http://localhost/shenieva-teacher/src/lib/api/fetch_quizzes.php?story=story2');
  ...
  async function addQuiz() {
    if (newQuestion.trim() && newAnswer.trim() && newPoints !== null) {
      try {
        const response = await fetch('http://localhost/shenieva-teacher/src/lib/api/add_quiz1.php?story=story2', {
          method: 'POST',
  ...
  async function deleteQuiz(id: number) {
    if (confirm("Are you sure you want to delete this quiz?")) {
      try {
        const response = await fetch(`http://localhost/shenieva-teacher/src/lib/api/delete_quiz.php?story=story2`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  async function updateQuiz() {
    if (editingQuizId !== null && newPoints !== null) {
      try {
        const response = await fetch('http://localhost/shenieva-teacher/src/lib/api/edit_quiz.php?story=story2', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            id: editingQuizId,
