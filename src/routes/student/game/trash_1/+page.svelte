<script lang="ts">
  ...
  async function handleOk() {
    ...
    try {
        const response = await fetch('http://localhost/shenieva-teacher/lib/api/collected_trash.php', {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            console.error('Failed to update trash:', response.status);
        }
    } catch (error) {
  ...
  onMount(async () => {
      ...
      if (gameCanvas) ctx = gameCanvas.getContext('2d') as CanvasRenderingContext2D;
