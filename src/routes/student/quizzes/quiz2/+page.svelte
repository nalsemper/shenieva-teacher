<script>
    ...
    import { createQuiz2Store } from '$lib/store/quiz2_taking';
    ...
    // Drag state (local, as it's transient)
    const draggedAnswer = writable(null);
  
    // Fetch quiz questions on mount
    onMount(async () => {
      try {
        const response = await fetch('http://localhost/shenieva-teacher/src/lib/api/student-story2/get_story1_quizzes.php');
        if (!response.ok) throw new Error('Failed to fetch quiz data');
        quizData = await response.json();
        quiz2Store = createQuiz2Store(quizData);
      } catch (err) {
        error = err.message;
      }
    });
    ...
    // Derived state
    $: allAnswered = quiz2Store ? $quiz2Store.answers.every(answer => answer.assignedTo !== null) : false;
    $: maxAttemptsReached = quiz2Store ? $quiz2Store.attempts >= $quiz2Store.maxAttempts : false;
    ...
    function handleDrop(event, question) {
      event.preventDefault();
      const hasAnswer = $quiz2Store.answers.some(answer => answer.assignedTo === question.id);
      if ($draggedAnswer && !hasAnswer) {
    ...
    function handleModalAction(event) {
      ...
      } else if (event.detail.action === 'retake' && !maxAttemptsReached) {
  ...
  {:else}
    <div class="h-screen w-screen bg-gradient-to-br from-indigo-200 via-purple-200 to-pink-200 p-8 flex flex-col items-center overflow-auto relative">
      ...
      <!-- Score and Points -->
      {#if $quiz2Store.score > 0 || $quiz2Store.totalPoints > 0}
      ...
      <!-- Modal -->
      {#if showModal}
        <QuizResultModal2 {quizData} on:action={handleModalAction} />
      {/if}
