<script lang="ts">
  ...
  // Archive student
  async function archiveStudent(student: any, event: Event) {
    event.stopPropagation(); // Prevent row click from opening modal
    
    if (!confirm(`Are you sure you want to archive ${student.studentName}? This will hide all their records.`)) {
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        alert(`Failed to archive student: ${result.message}`);
      }
    } catch (error) {
...
<div class="p-6 max-w-6xl mx-auto bg-white rounded-xl shadow-lg">
  <div class="flex items-center gap-4 mb-6">
    ...
    <a 
      href="/admin/students/archived" 
      class="bg-gray-500 text-white px-5 py-2 rounded-lg hover:bg-gray-600 shadow flex items-center gap-2"
    >
    ...
    <button 
      ...
      class="bg-orange-500 text-white px-5 py-2 rounded-lg hover:bg-orange-400 shadow"
    >
      + AddStudent
    </button>
  </div>
        ...
        {#each $attendees as student}
          ...
          >
            ...
            <td class="p-2 text-center">
              <button 
                on:click={(e) => archiveStudent(student, e)}
                disabled={isArchiving}
                class="bg-red-500 text-white px-4 py-1 rounded hover:bg-red-600 disabled:bg-gray-400 disabled:cursor-not-allowed text-xs"
              >
