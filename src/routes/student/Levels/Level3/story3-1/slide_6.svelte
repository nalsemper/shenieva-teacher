<script>
    ...
    // playlist: narration, dialogue, narration
    $: playlist = (() => {
        const base = '/assets/audio/story-telling/Level_3/story_1';
        const sp = speed === 'slow' ? 'slow' : (speed === 'fast' ? 'fast' : 'normal');
        let file1 = sp === 'slow' ? 'T5.mp3' : (sp === 'fast' ? 'TT5.mp3' : 'TT 5.mp3');
        let file3 = sp === 'slow' ? 'T6.mp3' : (sp === 'fast' ? 'TT6.mp3' : 'TT 6.mp3');
        return [
            `${base}/${sp}/slide_6/${file1}`,
            `${base}/${sp}/slide_6/Tonya 2.mp3`,
            `${base}/${sp}/slide_6/${file3}`
        ];
    })();
