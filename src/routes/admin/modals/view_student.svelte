...
<script lang="ts">
    ...
    import EditStudentModal from '$lib/tabs/edit_student.svelte'; // Import the new edit modal
    ...
    function handleEditSave(event) {
      ...
      dispatch('edit', updatedStudent); // Pass the edit event up to the parent
      showEditModal = false;
    }
    ...
    $: levelProgress = Math.min((parseInt(selectedPerson.studentLevel) || 0) / 3 * 100, 100);
  </script>
    ...
    >
      ...
      <div class="p-6 space-y-4">
        {#if activeTab === 'personal'}
          ...
          <div class="space-y-3">
            <div class="flex items-center gap-2">
              ...
              <span class="text-gray-900">{selectedPerson.studentId || 'N/A'}</span>
            </div>
            <div class="flex items-center gap-2">
              <span class="text-gray-600 font-medium min-w-[120px]">Name:</span>
              <span class="text-gray-900">{selectedPerson.studentName}</span>
            </div>
            <div class="flex items-center gap-2">
              <span class="text-gray-600 font-medium min-w-[120px]">Gender:</span>
              <span class="text-gray-900">{selectedPerson.studentGender}</span>
            </div>
          </div>
            ...
            <div>
              ...
              <div class="w-full bg-gray-200 rounded-full h-4">
                <div 
                  ...
                  aria-label={`Level: ${selectedPerson.studentLevel} out of 3`}
                ></div>
              </div>
              <span class="text-gray-700 text-sm mt-1 block">
                {selectedPerson.studentLevel} / 3
              </span>
            </div>
            <div>
        ...
        {:else if activeTab === 'attendance'}
          <div class="text-gray-700">
            Attendance data for {selectedPerson.studentName} will be displayed here.
          </div>
        {:else if activeTab === 'quiz1'}
          <div class="text-gray-700">
            Quiz 1 results for {selectedPerson.studentName} will be displayed here.
          </div>
        {:else if activeTab === 'quiz2'}
          <div class="text-gray-700">
            Quiz 2 results for {selectedPerson.studentName} will be displayed here.
          </div>
        {:else if activeTab === 'quiz3'}
          <div class="text-gray-700">
            Quiz 3 results for {selectedPerson.studentName} will be displayed here.
          </div>
        {/if}
      </div>
      ...
      <div class="p-4 border-t border-gray-200 flex justify-end gap-2">
        ...
        <button 
          type="button"
          on:click={handleClose} 
          class="bg-orange-500 text-white px-4 py-2 rounded-md hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-orange-400 focus:ring-offset-2"
        >
          Close
        </button>
      </div>
