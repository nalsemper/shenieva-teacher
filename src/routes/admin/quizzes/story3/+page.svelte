<script lang="ts">
  ...
  let quizzes = [
    { id: 1, question: "What is 2 + 2?", points: 5, expanded: false },
  ];
  ...
  function addQuiz() {
    if (newQuestion.trim() && newPoints !== null) {
      quizzes = [...quizzes, { id: Date.now(), question: newQuestion, points: newPoints, expanded: false }];
      closeModal();
    }
  }
  ...
  function updateQuiz() {
    if (editingQuizId !== null && newPoints !== null) {
      quizzes = quizzes.map(quiz => 
        quiz.id === editingQuizId ? { ...quiz, question: newQuestion, points: newPoints } : quiz
      );
      closeModal();
    }
  }
...
<div class="p-6 max-w-6xl mx-auto bg-white dark:bg-gray-900 shadow-lg rounded-lg">
  ...
  <div class="fixed inset-0 flex items-center justify-center bg-black/50">
    <div class="bg-white p-6 rounded-2xl shadow-2xl w-[700px] relative border-t-4 border-orange-500">
      ...
      <!-- Points Input with Floating Label -->
      <div class="relative w-full mb-4">
        <input 
          type="number" 
