<script>
    ...
    const slide = {
        ...
        image: '/converted/assets/LEVEL_2/STORY_2/PIC1.webp'
    };
    ...
    $: audioSrc = (() => {
        ...
        const filename = sp === 'slow' ? 'H1.mp3' : 'HM1.mp3';
        const path = `${base}/${sp}/slide_2/${filename}`;
        console.log('[slide_2] speed:', speed, 'sp:', sp, 'filename:', filename, 'path:', path);
        return path;
    })();
    ...
    $: if (audioEl && speed && audioSrc) {
        console.log('[slide_2] Reactive: speed changed, audioSrc:', audioSrc);
        enterStoryMode();
        const token = ++playToken;
        audioEl.pause();
        audioEl.src = audioSrc;
