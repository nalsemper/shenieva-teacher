...
try {
    ...
    if ($trashCollected !== null) {
        ...
        // Check if studentLevel is below 1 and update it to 1 if so
        if ($currentLevel < 1) {
            $newLevel = 1;
            ...
            // Update only studentColtrash if level is 1 or higher
    ...
    echo json_encode([
        ...
        'studentLevel' => $trashCollected !== null && $currentLevel < 1 ? 1 : (int)$currentLevel
    ]);
} catch (Exception $e) {
    $conn->rollback();
    http_response_code(500);
