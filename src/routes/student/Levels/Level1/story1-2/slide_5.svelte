<script>
    ...
    // Playlist - fast uses C4.mp3, normal/slow use C5.mp3, then Mother 2.mp3
    $: playlist = (() => {
        const base = '/assets/audio/story-telling/Level_1/story_2';
        if (speed === 'fast') {
            return [
                `${base}/fast/slide_5/C4.mp3`,
                `${base}/fast/slide_5/Mother 2.mp3`
            ];
        }
        const sp = speed === 'slow' ? 'slow' : 'normal';
        return [
            `${base}/${sp}/slide_5/C5.mp3`,
            `${base}/${sp}/slide_5/Mother 2.mp3`
        ];
    })();
