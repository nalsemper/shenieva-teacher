<script>
    ...
    // Playlist with 2 audio files per speed
    $: playlist = (() => {
        const base = '/assets/audio/story-telling/Level_1/story_3';
        const sp = speed === 'slow' ? 'slow' : (speed === 'fast' ? 'fast' : 'normal');
        
        // Fast speed uses H5 instead of H6
        const firstFile = (speed === 'fast') ? 'H5.mp3' : 'H6.mp3';
        
        return [
            `${base}/${sp}/slide_8/${firstFile}`,
            `${base}/${sp}/slide_8/Customer_.mp3`
        ];
    })();
